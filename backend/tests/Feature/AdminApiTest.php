...
it('reads and updates persistent system settings with an audit record', function () {
    ...
    $this->actingAs($admin)->putJson('/api/v1/admin/settings', [
        ...
        'welcome_background_image' => '/menu/burger-bundle.png',
    ])->assertOk()
        ->assertJsonPath('settings.brand_name', 'North Hall Kiosk')
        ->assertJsonPath('settings.welcome_background_image', '/menu/burger-bundle.png');

    $this->assertDatabaseHas('system_settings', ['id' => 1, 'brand_name' => 'North Hall Kiosk', 'welcome_background_image' => '/menu/burger-bundle.png']);
    $this->assertDatabaseHas('audit_logs', ['actor_id' => $admin->id, 'action' => 'settings.updated']);
});

it('uploads a welcome screen background image', function () {
    \Illuminate\Support\Facades\Storage::fake('public');
    $admin = User::factory()->create(['role' => 'admin']);

    $file = \Illuminate\Http\UploadedFile::fake()->image('background.jpg', 1920, 1080);
    $response = $this->actingAs($admin)->postJson('/api/v1/admin/settings/upload-background', [
        'image' => $file,
    ])->assertOk();

    $url = $response->json('url');
    expect($url)->toStartWith('/storage/backgrounds/');
    \Illuminate\Support\Facades\Storage::disk('public')->assertExists(str_replace('/storage/', '', $url));
});
