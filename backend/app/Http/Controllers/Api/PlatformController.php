...
class PlatformController extends Controller
{
    ...
    public function createStore(Request $request): JsonResponse
    {
        $data = $request->validate([
            ...
            'code' => ['required', 'string', 'max:32', 'alpha_dash', 'unique:stores,code'],
    ...
    public function updateStore(Request $request, int $storeId): JsonResponse
    {
        ...
        // Auto-revoke all active sessions for store users when store is disabled
        if (array_key_exists('active', $data) && ! $data['active']) {
            $userIds = DB::table('store_user')->where('store_id', $storeId)->pluck('user_id');
    ...
    public function users(): JsonResponse
    {
        $users = DB::table('users')
            ->leftJoin('store_user', 'store_user.user_id', '=', 'users.id')
            ->leftJoin('stores', 'stores.id', '=', 'store_user.store_id')
            ->select('users.id', 'users.name', 'users.email', 'users.role', 'users.created_at', 'stores.id as store_id', 'stores.name as store_name', 'store_user.role as store_role')
            ->orderBy('users.name')
            ->get();
    ...
    public function updateUser(Request $request, int $userId): JsonResponse
    {
        ...
        abort_if($user->id === $request->user()->id && $request->input('role') !== 'super_admin' && $request->has('role'), 422, 'You cannot remove your own Super Admin access.');

        $data = $request->validate([
            'name' => ['sometimes', 'string', 'max:120'],
            'role' => ['sometimes', 'in:super_admin,store_admin'],
        ...
        if (array_key_exists('store_id', $data)) {
            $existingStoreId = DB::table('store_user')->where('user_id', $user->id)->value('store_id');
            $storeChanged = $existingStoreId !== $data['store_id'];
        ...
        // Auto-revoke all sessions on role or store assignment change
    ...
    public function userSessions(int $userId): JsonResponse
    {
        $user = User::query()->findOrFail($userId);
    ...
    public function terminals(Request $request): JsonResponse
    {
        $query = DB::table('terminals')
            ->leftJoin('stores', 'stores.id', '=', 'terminals.store_id')
            ->select('terminals.*', 'stores.name as store_name')
            ->where('terminals.status', '!=', 'decommissioned');

        if ($request->filled('store_id')) {
            $query->where('terminals.store_id', $request->integer('store_id'));
        }

        return response()->json([
            'terminals' => $query->orderBy('terminals.id')->get()->map(function (object $terminal): object {
                $terminal->is_online = $terminal->last_heartbeat_at !== null && now()->diffInSeconds($terminal->last_heartbeat_at) < 60;
    ...
    public function report(Request $request): JsonResponse
    {
        ...
        if ($request->filled('from')) {
            $query->whereDate('orders.placed_at', '>=', $request->date('from'));
        }
        if ($request->filled('to')) {
            $query->whereDate('orders.placed_at', '<=', $request->date('to'));
        }
    ...
    public function impersonate(Request $request, int $userId): JsonResponse
    {
        ...
        $tokenObj = $target->createToken('impersonation-'.$sessionId, ['admin', 'impersonated'], now()->addMinutes(30));
