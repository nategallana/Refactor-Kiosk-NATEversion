...
class OrderController extends Controller
{
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            [$order, $canonicalItems, $replayed] = DB::transaction(function () use ($data, $idempotencyKey, $requestFingerprint): array {
                $existing = DB::table('orders')->where('idempotency_key', $idempotencyKey)->lockForUpdate()->first();
                if ($existing !== null) {
                    [$existing, $existingItems] = $this->replayExistingOrder($existing, $requestFingerprint);

                    return [$existing, $existingItems, true];
                }
            ...
            [$order, $canonicalItems] = $this->replayExistingOrder($existing, $requestFingerprint);
            $replayed = true;
        }
        ...
        return response()
            ->json(['order' => $order], $replayed ? 200 : 201)
            ->header('Idempotency-Replayed', $replayed ? 'true' : 'false');
    }

    /**
     * @return array{0: object, 1: array<int, array<string, mixed>>}
     */
